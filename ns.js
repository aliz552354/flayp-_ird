...
function setHighScore() {
    var e = $("#highscore");
    e.empty();
    for (var t = highscore.toString().split(""), o = 0; o < t.length; o++)
        e.append("<img src='assets/font_small_" + t[o] + ".png' alt='" + t[o] + "'>")
}
function setMedal() {
    var e = $("#medal");
...
function showScore() {
    ...
    setBigScore(!0),
    score > highscore && setCookie("highscore", highscore = score, 999),
    setSmallScore(),
    setHighScore();
    var e = setMedal();
    soundSwoosh.stop(),
...
ezBtn.addEventListener("click", function() {
    ezMode ? (ezBtn.innerText = "Easy",
    pipeheight = 90) : (ezBtn.innerText = "Hard",
    pipeheight = 200),
    ezMode = !ezMode
}),
$(document).ready(function() {
    "?debug" == window.location.search && (debugmode = !0),
    "?easy&mode=full" == window.location.search && (pipeheight = 200);
...
$(document).ready(function() {
    ...
    var $muteButton = $("#mute");
    var $unmuteIcon = $muteButton.find(".icon-unmuted");
    var $muteIcon = $muteButton.find(".icon-muted");
    ...
    function updateMuteButtonState(isMuted) {
        ...
        if (soundJump)
            soundJump.setVolume(volume);
        if (soundScore)
            soundScore.setVolume(volume);
        if (soundHit)
            soundHit.setVolume(volume);
        if (soundDie)
            soundDie.setVolume(volume);
        if (soundSwoosh)
            soundSwoosh.setVolume(volume);
    }
    ...
    // More games button
    $("#more-game").on("click", function(e) {
        e.preventDefault();
        window.open("https://retrogames.ee", "_blank");
    });
});
